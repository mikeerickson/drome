...
const run = async (task, name, cliOptions = []) => {
    ...
    let cmd = (typeof name === 'undefined') ? 'help' : '';
    if (typeof task === 'undefined') {
        cmd = hasOption(name, ['-H', '-h', '--help', 'help']) ? 'help' : cmd;
        cmd = hasOption(name, ['-V', '-v', '--version', 'version']) ? 'version' : cmd;
        cmd = hasOption(name, ['-L', '-l', '--logs', 'logs']) ? 'logs' : cmd;
    }
