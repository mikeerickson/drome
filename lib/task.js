...
const { sanitizeCommand, hasOption, showHelp, showVersion } = require('./utils');
...
const run = async (task, name, cliOptions = []) => {
    let cmd = '';
    if (typeof task === 'undefined') {
        cmd = hasOption(name, ['-H', '--help', 'help']) ? 'help' : cmd;
        cmd = hasOption(name, ['-V', '--version', 'version']) ? 'version' : cmd;
    }
    if (cmd === 'version' || cmd === 'help') {
        await cliCommand(cmd);
    }
