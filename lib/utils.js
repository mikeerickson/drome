...
const getTaskList = () => {
    ...
    if (fs.existsSync(homeConfigPath)) {
        ...
        // create global config, then load
    ...
    if (typeof projectConfig === 'function') {
        allTasks = Object.assign(homeConfig().tasks, projectConfig().tasks);
    } else {
        allTasks = Object.assign(homeConfig().tasks);
    }
    return allTasks;
};

const wordwrap = function (str, maxChars, lineEnd = '\n') {

    let sum_length_of_words = function (word_array) {
        let out = 0;
    ...
    let splitLongWord = function (word, maxChar) {
        let out = [];
        if (maxChar >= 1) {
